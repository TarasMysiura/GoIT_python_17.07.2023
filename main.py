...
def create_enemy():    
    enemy = pygame.transform.scale(pygame.image.load(
        'enemy.png').convert_alpha(), (100, 30))
    enemy_rect = pygame.Rect(
        width, random.randint(50, height - 150), *enemy.get_size())
    enemy_speed = random.randint(2, 5)
    return [enemy, enemy_rect, enemy_speed]

def create_bonus():    
    bonus = pygame.transform.scale(pygame.image.load(
        'bonus.png').convert_alpha(), (70, 100))
    bonus_rect = pygame.Rect(
        random.randint(100, width - 100), 0, *bonus.get_size())
